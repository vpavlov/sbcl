...
(define-vop (move-from-signed)
  ...
  (:generator 20
	...
	(storew x y bignum-digits-offset other-pointer-lowtag))
      (inst b done)
      ;; fixnum case. just shift it left 2 bits
      (emit-label fixnum)
      (inst lsl y x n-fixnum-bits)
      (emit-label done))))
;;;
...
(define-vop (move-from-unsigned)
  ...
  (:generator 20
    ...
    (let ((done (gen-label))
          (one-word (gen-label)))
      (inst asrs temp x n-positive-fixnum-bits)
      (inst lsl y x n-fixnum-tag-bits)
      (inst b done :cnd :eq)
      (with-fixed-allocation (y temp temp2 bignum-widetag
...
(define-vop (word-move)
  (:args (x :target y :scs (signed-reg unsigned-reg)
	    :load-if (not (location= x y))))
  (:results (y :scs (signed-reg unsigned-reg)
               :load-if (not (location= x y))))
  (:effects)

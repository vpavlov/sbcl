...
(define-vop (lowtag-of)
  ...
  (:generator 1
    (inst ands result object lowtag-mask)))
...
(define-vop (widetag-of)
  ...
  (:generator 6
    ...
    (inst cmp a0-tn result list-pointer-lowtag)
    (inst b done :cnd :eq)
    (inst cmp a0-tn result other-pointer-lowtag)
    (inst b other-pointer :cnd :eq)
    (inst cmp a0-tn result fun-pointer-lowtag)
    (inst b function-pointer :cnd :eq)
    (inst cmp a0-tn result instance-pointer-lowtag)
    (inst b done :cnd :eq)
    ...
    (inst ands result object widetag-mask)
    (inst b done)
...
(define-vop (set-fun-subtype)
  ...
  (:result-types positive-fixnum)
  (:generator 6
    (inst strb type function (- 3 fun-pointer-lowtag))
    (move result type)))
...
(define-vop (set-header-data)
  ...
  (:generator 6
    ...
    (inst ands t1 t1 widetag-mask)
    (sc-case data
      (any-reg
       (inst lsr t2 data (- n-widetag-bits n-fixnum-tag-bits))
       (inst eor t1 t1 t2))
      (immediate
       (inst eor t1 t1 (ash (tn-value data) n-widetag-bits))))
...
(define-vop (pointer-hash)
  ...
  (:generator 1
	      (move res ptr)
	      (inst and res res (lognot lowtag-mask))
	      (inst lsr res res 1)))
...
(define-vop (current-thread-offset-sap)
  ...
  (:policy :fast-safe)
  (:generator 2
    (inst lsl n n word-shift)
    (inst ldr sap thread-base-tn n)))
...
(define-vop (count-me)
  ...
  (:generator 1
    (let ((offset
      ...
      (inst ldr count count-vector offset)
      (inst add count count 1)
      (inst str count count-vector offset))))

